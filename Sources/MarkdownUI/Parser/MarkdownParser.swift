...
extension Array where Element == BlockNode {
    ...
    func renderMarkdown() -> String {
        ...
        return LatexParser.removeNewLinePlaceholder(text: markdown)
    }
    
    func renderPlainText() -> String {
        let plainText = UnsafeNode.makeDocument(self) { document in
            String(cString: cmark_render_plaintext(document, CMARK_OPT_DEFAULT, 0))
        } ?? ""
        guard plainText.isEmpty == false else { return plainText }
        return LatexParser.removeNewLinePlaceholder(text: plainText)
    }
    
    func renderHTML() -> String {
        let html = UnsafeNode.makeDocument(self) { document in
            String(cString: cmark_render_html(document, CMARK_OPT_DEFAULT, nil))
        } ?? ""
        guard html.isEmpty == false else { return html }
        return LatexParser.removeNewLinePlaceholder(text: html)
    }
}
...
extension BlockNode {
    fileprivate init?(unsafeNode: UnsafeNode) {
        ...
        case .htmlBlock:
            var literal = unsafeNode.literal ?? ""
            literal = LatexParser.removeNewLinePlaceholder(text: literal)
            self = .htmlBlock(content: literal)
        case .paragraph:
...
extension InlineNode {
    ...
    fileprivate init?(unsafeNode: UnsafeNode) {
        switch unsafeNode.nodeType {
        case .text:
            if let literal = unsafeNode.literal, literal.isEmpty == false {
                self = InlineNode.parseMarkdownToInlineNodes(literal)
            } else {
                self = .text("")
            }
        case .softBreak:
    ...
    fileprivate static func parseMarkdownToInlineNodes(_ text: String) -> InlineNode {
        var nodes: [LatexNode] = []
        let ranges = LatexParser.findLatexRanges(in: text)
        guard ranges.isEmpty == false else { return InlineNode.text(text) }
        var start = LatexParser(range: ranges[0], tag: text[ranges[0].lowerBound..<ranges[0].upperBound])
        var textFind = text.startIndex
        for (index, item) in ranges.enumerated() {
            if start.range != item {
                var isLatex = false
                let itemLatex = LatexParser(range: item, tag: text[item.lowerBound..<item.upperBound])
                if start.pairing(parser: itemLatex) {
                    let latex = text[start.range.upperBound..<itemLatex.range.lowerBound]
                    if latex.isEmpty == false {
                        let isBlock = start.isBlock(parser: itemLatex, text: text)
                        if textFind < start.range.lowerBound {
                            let rawText = text[textFind..<start.range.lowerBound]
                            if rawText.isEmpty == false {
                                nodes.append(LatexNode.text(String(rawText)))
                            }
                        }
                        let latex = LatexNode.latex(content: LatexParser.removeNewLinePlaceholder(text: String(latex)), isBlock: isBlock)
                        nodes.append(latex)
                        textFind = itemLatex.range.upperBound
                        isLatex = true
                    }
                }
                if isLatex {
                    if index < ranges.count - 1 {
                        ...
                        start = LatexParser(range: next, tag: text[next.lowerBound..<next.upperBound])
                    }
                } else {
                    start = itemLatex
                }
        ...
        if textFind != text.endIndex {
            let rawText = text[textFind..<text.endIndex]
            if rawText.isEmpty == false {
                nodes.append(LatexNode.text(String(rawText)))
            }
        }
        if nodes.count == 1, case .text(_) = nodes[0] {
            return InlineNode.text(text)
        }
        return InlineNode.latex(rawContent: text, nodes)
    }
}
